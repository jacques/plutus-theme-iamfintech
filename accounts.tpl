...
{include file="header.tpl" title="Accounts Page" nav="accounts"}
      ...
      <div>
        <h2 class="page-header">Demo Dashboard</h2>
      </div>
...
{if $row.account_type eq 'wallet'}
              <td><i class="fa fa-money"></i> Demo Account &mdash; {$row.account_number} <div class="pull-right"><a href="/accounts/{$row.uuid}/transactions">View Transactions</a> &bull; <a href="/accounts/{$row.uuid}/statements">View statements</a></div></td>
              <td><div class="pull-right">R {($row.balance/100)|string_format:"%.2f"}</div></td>
{else}
              <td><i class="fa fa-credit-card"></i> Demo Card Association Card &mdash; {$row.masked_cardnumber} <div class="pull-right"></div></td>
              <td>&nbsp;</td>
{/if}
            </tr>
{/foreach}
{foreach $business_accounts item=row}
            <tr>
              <td><i class="fa fa-money"></i> Demo Business Account &mdash; {$row.account_number} <div class="pull-right"><a href="/businessaccounts/{$row.uuid}/users">View Users</a> &bull; <a href="/businessaccounts/{$row.uuid}/transactions">View Transactions</a> &bull; <a href="/businessaccounts/{$row.uuid}/statements">View statements</a></div></td>
              <td><div class="pull-right">R {($row.balance/100)|string_format:"%.2f"}</div></td>
            </tr>
{/foreach}
          </tbody>
